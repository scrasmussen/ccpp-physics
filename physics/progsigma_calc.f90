...
!     Local variables
      ...
      real(kind=kind_phys) :: ZZ,cvg,mcon,buy2
      real(kind=kind_phys) :: fdqb,dtdyn,dxlim
      real(kind=kind_phys) :: tem,DEN,dp1

     !Parameters
      real(kind_phys), parameter :: one = 1.0
      real(kind_phys), parameter :: zero = 0.0
      real(kind_phys), parameter :: gcvalmx = 0.1
      real(kind_phys), parameter :: rmulacvg=10.
      real(kind_phys), parameter :: epsilon=1.E-11
      real(kind_phys), parameter :: betadcu = 2.0
      real(kind_phys), parameter :: betascu = 8.0
      real(kind_phys)            :: invdelt
      integer                    :: km1
      invdelt = 1./delt
      km1=km-1
      ...
      if(flag_init .and. .not. flag_restart)then
         ...
         do i = 1,im
            if(cnvflg(i))then
               ...
               cvg=termD(i)*delt
               ZZ=MAX(0.0,SIGN(one,termA(i)))            &
                    *MAX(0.0,SIGN(one,termB(i)))         &
                    *MAX(0.0,SIGN(one,termC(i)-epsilon))
               cvg=MAX(0.0,cvg)
