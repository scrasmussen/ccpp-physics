...
module scm_mp_wsm6
      ...
      public :: scm_mp_wsm6_init, scm_mp_wsm6_run, scm_mp_wsm6_finalize
...
!!
     subroutine scm_mp_wsm6_init(den0, denr, dens, cl, &
      ...
      end subroutine scm_mp_wsm6_init


!> \section arg_table_scm_mp_wsm6_run Argument Table
!! \htmlinclude scm_mp_wsm6_run.html
!!
!>\ingroup aathompson
!>\section gen_thompson_hrrr Thompson MP General Algorithm
!>@{
      subroutine scm_mp_wsm6_run(t,q,qc,qi,qr,qs,qg,den,p,delz,delt,   &
         ...
         real(kind=kind_phys),intent(inout),dimension(its:ite,kts:kte):: qc
         real(kind=kind_phys),intent(inout),dimension(its:ite,kts:kte):: qi
         real(kind=kind_phys),intent(inout),dimension(its:ite,kts:kte):: qr
         real(kind=kind_phys),intent(inout),dimension(its:ite,kts:kte):: qs
         real(kind=kind_phys),intent(inout),dimension(its:ite,kts:kte):: qg
         real(kind=kind_phys),intent(inout),dimension(its:ite):: rain
         real(kind=kind_phys),intent(inout),dimension(its:ite):: rainncv
         real(kind=kind_phys),intent(inout),dimension(its:ite):: sr
         real(kind=kind_phys),intent(inout),dimension(its:ite),optional:: snow
         real(kind=kind_phys),intent(inout),dimension(its:ite),optional:: snowncv
         real(kind=kind_phys),intent(inout),dimension(its:ite),optional:: graupel
         real(kind=kind_phys),intent(inout),dimension(its:ite),optional:: graupelncv
         real(kind=kind_phys),intent(inout),dimension(its:ite,kts:kte),optional :: rainprod2d
         real(kind=kind_phys),intent(inout),dimension(its:ite,kts:kte),optional :: evapprod2d
         ! CCPP error handling
         character(len=*),          intent(  out) :: errmsg
         integer,                   intent(  out) :: errflg
         ...
         if (.not. is_initialized) then
            write(errmsg, fmt='((a))') 'scm_mp_wsm6_run called before scm_mp_wsm6_init'
            errflg = 1
            return
         end if

         call mp_wsm6_run(t,q,qc,qi,qr,qs,qg,den,p,delz,delt,   &
                          g,cpd,cpv,rd,rv,t0c,ep1,ep2,qmin,xls, &
                          xlv0,xlf0,den0,denr,cliq,cice,psat,   &
                          rain,rainncv,sr, &
                          snow=snow, snowncv=snowncv, &
                          graupel=graupel, graupelncv=graupelncv, &
                          rainprod2d=rainprod2d, evapprod2d=evapprod2d, &
                          its=its, ite=ite, kts=kts, kte=kte, &
                          errmsg=errmsg, errflg=errflg &
                         )
      end subroutine scm_mp_wsm6_run
!>@}

!> \section arg_table_scm_mp_wsm6_finalize Argument Table
!! \htmlinclude scm_mp_wsm6_finalize.html
!!
      subroutine scm_mp_wsm6_finalize(errmsg, errflg)
      ...
      end subroutine scm_mp_wsm6_finalize

end module scm_mp_wsm6
